...
static const vshCmdOptDef opts_migrate[] = {
    ...
    {.name = "paused",
     .type = VSH_OT_BOOL,
     .help = N_("pause guest in the source host after migration")
    },
    {.name = "local",
     .type = VSH_OT_BOOL,
     .help = N_("localhost migration")
    },
    {.name = NULL}
};
...
doMigrate(void *opaque)
{
    ...
    if (vshCommandOptBool(cmd, "paused"))
        flags |= VIR_MIGRATE_PAUSED_SOURCE;

    if (vshCommandOptBool(cmd, "local"))
        flags |= VIR_MIGRATE_LOCALHOST;
...
/*
 * "migrate-localhost" command
 */
static const vshCmdInfo info_migrate_localhost[] = {
    {.name = "help",
     .data = N_("migrate domain to local host")
    },
    {.name = "desc",
     .data = N_("Live migrate domain to local host to update qemu")
    },
    {.name = NULL}
};

static const vshCmdOptDef opts_migrate_localhost[] = {
    VIRSH_COMMON_OPT_DOMAIN_FULL,
    {.name = NULL}
};

static bool
cmdMigrateLocalhost(vshControl *ctl, const vshCmd *cmd)
{
    const char *n = NULL;
    ...
    if (virAsprintf(&cmdStr, "migrate %s qemu+unix:///system --local --live --unsafe", n) < 0)
        goto cleanup;

    /* Avoid being released early */
    ctl->cmd = NULL;
    if (!vshCommandStringParse(ctl, cmdStr))
        goto cleanup;

    ret = cmdMigrate(ctl, ctl->cmd);
...
const vshCmdDef domManagementCmds[] = {
    ...
    {.name = "migrate-localhost",
     .handler = cmdMigrateLocalhost,
     .opts = opts_migrate_localhost,
     .info = info_migrate_localhost,
     .flags = 0
    },
