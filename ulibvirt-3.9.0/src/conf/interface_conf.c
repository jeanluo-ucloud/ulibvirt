...
static int
virInterfaceDefParseTXQ(virInterfaceDefPtr def,
                        xmlXPathContextPtr ctxt)
{
    unsigned long txqueuelen;
    int ret;

    ret = virXPathULong("string(./txqueuelen/@size)", ctxt, &txqueuelen);
    if ((ret == -2) || ((ret == 0) && (txqueuelen > 100000))) {
        virReportError(VIR_ERR_XML_ERROR,
                       "%s", _("interface txqueuelen value is improper"));
        return -1;
    } else if (ret == 0) {
        def->txqueuelen = (unsigned int) txqueuelen;
    }
                        ...
                        int parentIfType)
{
    ...
    if (parentIfType == VIR_INTERFACE_TYPE_LAST) {
        ...
        if (virInterfaceDefParseMtu(def, ctxt) < 0)
            goto error;
        if (virInterfaceDefParseTXQ(def, ctxt) < 0)
            goto error;
