...
typedef struct _virDomainVhostUserDef virDomainVhostUserDef;
typedef virDomainVhostUserDef *virDomainVhostUserDefPtr;

typedef struct _virDomainVhostUserMemDef virDomainVhostUserMemDef;
typedef virDomainVhostUserMemDef *virDomainVhostUserMemDefPtr;
...
typedef enum {
    ...
    VIR_DOMAIN_DEVICE_VHOSTUSER,
    VIR_DOMAIN_DEVICE_VHOSTUSERMEM,
...
typedef enum {
    VIR_DOMAIN_QEMU_CPU_LEGACY_CACHE = 0,
    VIR_DOMAIN_QEMU_CPU_XLEVEL,

    VIR_DOMAIN_QEMU_CPU_LAST
} virDomainQemuCpu;
...
struct _virDomainVhostUserMemDef {
    char id[1024];
    char path[1024];
    char size[1024];
    char align[1024];
...
struct _virDomainDef {
    ...
    int qemu_cpu_features[VIR_DOMAIN_QEMU_CPU_LAST];
    unsigned int qemu_cpu_xlevel;
    ...
    size_t nvhostusers;
    virDomainVhostUserDefPtr *vhostusers;
...
struct _virDomainObj {
    ...
    virDomainObjPtr dst; /* Localhost migration */
};
...
ssize_t virDomainInputDefFind(const virDomainDef *def,
    ...
    ATTRIBUTE_NONNULL(1) ATTRIBUTE_NONNULL(2) ATTRIBUTE_RETURN_CHECK;
ssize_t virDomainVhostUserDefFind(const virDomainDef *def,
                              const virDomainVhostUserDef *input)
    ATTRIBUTE_NONNULL(1) ATTRIBUTE_NONNULL(2) ATTRIBUTE_RETURN_CHECK;
...
VIR_ENUM_DECL(virDomainQemuCpu)
