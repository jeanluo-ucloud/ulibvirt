                           ...
                           virQEMUDriverPtr driver)
{
    ...
    case VIR_DOMAIN_DEVICE_VHOSTUSER:
        vhostuser = dev->data.vhostuser;
        if (virDomainVhostUserDefFind(vm->def, vhostuser) >= 0) {
            virReportError(VIR_ERR_OPERATION_FAILED, "matching vhostuer device id:%s and path:%s exists",
                           vhostuser->id, vhostuser->path);
            return -1;
        }
        ret = qemuDomainAttachVhostUserDevice(driver, vm, dev->data.vhostuser);
        if (ret == 0) {
	    dev->data.vhostuser = NULL;
        }
        break;
                           ...
                           virQEMUDriverPtr driver)
{
    ...
    case VIR_DOMAIN_DEVICE_INPUT:
        ...
        break;
    case VIR_DOMAIN_DEVICE_VHOSTUSER:
        if (virDomainVhostUserDefFind(vm->def, dev->data.vhostuser) < 0) {
            virReportError(VIR_ERR_OPERATION_FAILED, "matching vhostuer device id:%s and path:%s not found",
                           dev->data.vhostuser->id, dev->data.vhostuser->path);
            return -1;
        }
        ret = qemuDomainDetachVhostUserDevice(vm, dev->data.vhostuser);
        break;
                             ...
                             virDomainXMLOptionPtr xmlopt)
{
    ...
    case VIR_DOMAIN_DEVICE_VHOSTUSER:
        vhostuser = dev->data.vhostuser;
        if (virDomainVhostUserDefFind(vmdef, vhostuser) >= 0) {
            virReportError(VIR_ERR_OPERATION_FAILED, "matching vhostuer device id:%s and path:%s exists",
                           vhostuser->id, vhostuser->path);
            return -1;
        }

        if (VIR_APPEND_ELEMENT(vmdef->vhostusers, vmdef->nvhostusers, vhostuser) < 0)
            return -1;
        VIR_DEBUG("AttachVhostUserConfig, nvhostusers:%lu", vmdef->nvhostusers);
        dev->data.vhostuser = NULL;
        break;
                             ...
                             virDomainXMLOptionPtr xmlopt)
{
    ...
    case VIR_DOMAIN_DEVICE_VHOSTUSER:
        if ((idx = virDomainVhostUserDefFind(vmdef, dev->data.vhostuser)) < 0) {
            virReportError(VIR_ERR_OPERATION_FAILED, "matching vhostuer device id:%s and path:%s not found",
                           dev->data.vhostuser->id, dev->data.vhostuser->path);
            return -1;
        }
        VIR_DELETE_ELEMENT(vmdef->vhostusers, idx, vmdef->nvhostusers);
	break;
                         ...
                         int cancelled)
{
    ...
    if (flags & VIR_MIGRATE_LOCALHOST) {
        vm = vm->dst;
        virObjectRef(vm);
        virObjectLock(vm);
    }
                     ...
                     virDomainDiskDefPtr disk)
{
    ...
    if (disk->mirrorJob == VIR_DOMAIN_BLOCK_JOB_TYPE_COPY) {
        ...
        if (disk->src->type != VIR_STORAGE_TYPE_NETWORK &&
            qemuDomainDetermineDiskChain(driver, vm, disk, false, true) < 0)
            goto cleanup;
                          ...
                          bool keepParentLabel)
{
    ...
    if (mirror->type != VIR_STORAGE_TYPE_NETWORK &&
    ...
    if (mirror->type != VIR_STORAGE_TYPE_NETWORK &&
        qemuDomainStorageFileInit(driver, vm, mirror) < 0)
        goto endjob;

    if (mirror->type != VIR_STORAGE_TYPE_NETWORK &&
        qemuDomainBlockCopyValidateMirror(mirror, disk->dst, &reuse) < 0)
        goto endjob;
    ...
    if (mirror->type == VIR_STORAGE_TYPE_NETWORK) {
        if (virAsprintf(&buffer, "udisk:%s:%d:%s",
                        mirror->hosts->name,
                        mirror->hosts->port,
                        mirror->path) < 0)
            goto endjob;
    ...
    ret = qemuMonitorDriveMirror(priv->mon, device, buffer ? buffer : mirror->path, format,
