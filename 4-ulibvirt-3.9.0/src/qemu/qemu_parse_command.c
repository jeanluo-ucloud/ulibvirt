    ...
    for (i = 0; i < nkeywords; i++) {
        if (STREQ(keywords[i], "file")) {
            if (values[i] && STRNEQ(values[i], "")) {
                ...
                } else if (STRPREFIX(def->src->path, "udisk:")) {
                    char *p = def->src->path;
                    char *port, *vdi;

                    def->src->type = VIR_STORAGE_TYPE_NETWORK;
                    def->src->protocol = VIR_STORAGE_NET_PROTOCOL_UDISK;
                    if (VIR_STRDUP(def->src->path, p + strlen("udisk:")) < 0)
                        goto error;
                    VIR_FREE(p);
                    ...
                    if (port) {
                        *port = '\0';
                        ...
                        if (!vdi) {
                            *port = ':';
                            virReportError(VIR_ERR_INTERNAL_ERROR,
                                           _("cannot parse udisk filename '%s'"),
                                           def->src->path);
                            goto error;
                        }
                        port++;
                        ...
                        const char *val)
{
    ...
    for (i = 0; tokens[i] != NULL; i++) {
        ...
        if (i == 0) {
        ...
        } else if (STRPREFIX(tokens[i], "legacy-cache") ||
                   STRPREFIX(tokens[i], "xlevel")) {
            const char *feature, *value;
            int f;
            ...
            /* reuse hv_tokens, better rename hv_tokens */
            if (!(hv_tokens = virStringSplit(tokens[i], "=", 2)))
                goto cleanup;
            ...
            if ((f = virDomainQemuCpuTypeFromString(feature)) < 0) {
                virReportError(VIR_ERR_CONFIG_UNSUPPORTED,
                               _("unsupported qemu cpu feature '%s'"), feature);
                goto cleanup;
            }

            switch ((virDomainQemuCpu) f) {
            case VIR_DOMAIN_QEMU_CPU_LEGACY_CACHE:
                if ((dom->qemu_cpu_features[f] = virTristateSwitchTypeFromString(value)) < 0) {
                    virReportError(VIR_ERR_CONFIG_UNSUPPORTED,
                                   _("qemu cpu feature '%s' should be 'on', "
                                     "'off' or empty"), feature);
                    goto cleanup;
                }
                break;

            case VIR_DOMAIN_QEMU_CPU_XLEVEL:
                dom->qemu_cpu_features[f] = VIR_TRISTATE_SWITCH_ON;
                if (!value) {
                    virReportError(VIR_ERR_CONFIG_UNSUPPORTED, "%s",
                                   _("missing qemu cpu xlevel"));
                    goto cleanup;
                }


                if (virStrToLong_ui(value, NULL, 0, &dom->qemu_cpu_xlevel) < 0) {
                    virReportError(VIR_ERR_CONFIG_UNSUPPORTED, "%s",
                                   _("cannot parse qemu cpu xlevel"));
                    goto cleanup;
                }
    ...
    for (i = 1; progargv[i]; i++) {
        ...
        if (STREQ(arg, "-vnc")) {
            ...
            if (STRPREFIX(val, "/dev/")) {
                ...
                val += strlen("sheepdog:");
            } else if (STRPREFIX(val, "udisk:")) {
                disk->src->type = VIR_STORAGE_TYPE_NETWORK;
                disk->src->protocol = VIR_STORAGE_NET_PROTOCOL_UDISK;
                val += strlen("udisk:");
                ...
                case VIR_STORAGE_NET_PROTOCOL_UDISK:
                    ...
                    if (port) {
                        ...
                        *port++ = '\0';
                        ...
                        if (!vdi) {
                            virReportError(VIR_ERR_INTERNAL_ERROR,
                                           _("cannot parse udisk filename '%s'"), val);
                            goto error;
                        }
                        *vdi++ = '\0';
                        if (VIR_ALLOC(disk->src->hosts) < 0)
