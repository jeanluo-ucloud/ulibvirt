                               ...
                               virDomainPCIAddressSetPtr addrs)
{
    ...
    for (i = 0; i < def->nvhostusers; i++) {
        virDomainVhostUserDefPtr vhostuser = def->vhostusers[i];

        if (!virDeviceInfoPCIAddressWanted(&vhostuser->info)) {
            continue;
        }

        if (qemuDomainPCIAddressReserveNextAddr(addrs, &vhostuser->info) < 0)
            goto error;
    }
