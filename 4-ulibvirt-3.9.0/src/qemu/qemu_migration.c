                         ...
                         virConnectPtr dconn)
{
    ...
    for (i = 0; i < vm->def->ndisks; i++) {
        ...
        if (disk->src->protocol == VIR_STORAGE_NET_PROTOCOL_UDISK)
            continue;
                        ...
                        unsigned long flags)
{
    ...
    if (flags & VIR_MIGRATE_LOCALHOST) {
        ...
        if (!(src = virDomainObjListFindByName(driver->domains, origname ? origname : vm->def->name)))
            goto cleanup;
        src->dst = vm;

        virDomainObjEndAPI(&src);
    } else if (!(vm = virDomainObjListAdd(driver->domains, *def,
                                          driver->xmlopt,
    ...
    *def = NULL;
    ...
    if (flags & VIR_MIGRATE_LOCALHOST) {
        if (VIR_ALLOC(incoming) < 0)
            goto stopjob;

        if (virAsprintf(&incoming->launchURI, "unix:%s/incoming.sock", priv->libDir) < 0)
            goto stopjob;
        incoming->fd = -1;
    } else if (!(incoming = qemuMigrationPrepareIncoming(vm, tunnel, protocol,
                                                      listenAddress, port,
                                                      dataFD[0])))
        goto stopjob;
                          ...
                          int retcode)
{
    ...
    if (retcode == 0) {
        ...
        if (!(flags & VIR_MIGRATE_PAUSED_SOURCE)) {
        ...
        ignore_value(virPidFileWritePath(priv->pidfile, vm->pid));
        if (virDomainSaveStatus(driver->xmlopt, cfg->stateDir, vm, driver->caps) < 0)
            VIR_WARN("Failed to save status on vm %s", vm->def->name);
    }
                     ...
                     int cancelled)
{
    ...
    if (!virDomainObjIsActive(vm)) {
        ...
        if (flags & VIR_MIGRATE_LOCALHOST) {
            ...
            qemuDomainObjPrivatePtr priv = dst->privateData;

            virObjectRWLockWrite(driver->domains);

            virUUIDFormat(dst->def->uuid, uuidstr);
            virHashUpdateEntry(driver->domains->objs, uuidstr, dst);
            virHashUpdateEntry(driver->domains->objsName, dst->def->name, dst);

            virObjectRef(dst);
            virObjectRWUnlock(driver->domains);

            ignore_value(virPidFileWritePath(priv->pidfile, dst->pid));
            ignore_value(virDomainSaveStatus(driver->xmlopt, cfg->stateDir, dst, driver->caps));
        }
    }
                 ...
                 qemuMonitorMigrationParamsPtr migParams)
{
    ...
    case MIGRATION_DEST_UNIX:
        {
            ...
            int retries = 50;
            do {
                if (access(spec->dest.unix_socket.file, 0) != -1)
                    break;
            } while ((++i <= retries) && usleep(100 * 1000) <= 0);
        }

        rc = qemuMonitorMigrateToUnix(priv->mon, migrate_flags,
 ...
 cleanup:
    ...
    if (spec->destType == MIGRATION_DEST_UNIX) {
        unlink(spec->dest.unix_socket.file);
                           ...
                           qemuMonitorMigrationParamsPtr migParams)
{
    ...
    if (flags & VIR_MIGRATE_LOCALHOST) {
        qemuDomainObjPrivatePtr dstpriv = vm->dst->privateData;

        spec.destType = MIGRATION_DEST_UNIX;
        if(virAsprintf(&spec.dest.unix_socket.file, "%s/incoming.sock", dstpriv->libDir) < 0)
            goto cleanup;
        spec.fwdType = MIGRATION_FWD_DIRECT;
        /* restore pid because of dst vm update the pidfile */
        ignore_value(virPidFileWritePath(priv->pidfile, vm->pid));
    }
