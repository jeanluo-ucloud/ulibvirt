...
unsigned long long qemuDomainRemoveDeviceWaitTime = 1000ull * 10;
...
int
qemuDomainAttachVhostUserDevice(virQEMUDriverPtr driver,
                            virDomainObjPtr vm,
                            virDomainVhostUserDefPtr vhostuser)
{
    ...
    virDomainDeviceDef dev = { VIR_DOMAIN_DEVICE_VHOSTUSER,
                               { .vhostuser = vhostuser } };
    if (qemuDomainEnsurePCIAddress(vm, &dev, driver) < 0) {
        virReportError(VIR_ERR_INTERNAL_ERROR, "%s", _("can not ensure vhostuser pci address"));
        return ret;
    }

    VIR_DEBUG("allocate vhost-user-blk device pci address slot:%d", vhostuser->info.addr.pci.slot);

    devstr = qemuBuildVhostUserDeviceStr(vhostuser, vm->def, priv->qemuCaps);
    if (devstr == NULL)
        goto free_pci_addr;
    
    qemuDomainObjEnterMonitor(driver, vm);
    if (qemuMonitorAttachCharDev(priv->mon, vhostuser->id, vhostuser->source) < 0) {
    	virReportError(VIR_ERR_INTERNAL_ERROR, "%s", _("attach vhost-user-blk chrdev failed"));
        goto exit_monitor;
    }

    if (qemuMonitorAddDevice(priv->mon, devstr) < 0) {
    	virReportError(VIR_ERR_INTERNAL_ERROR, "%s", _("attach vhost-user-blk device failed, detach vhost-user-blk chrdev"));
	if (qemuMonitorDetachCharDev(priv->mon, vhostuser->id) < 0)
    	    virReportError(VIR_ERR_INTERNAL_ERROR, "%s", _("detach vhost-user-blk chrdev failed"));
        goto exit_monitor;
    }

    if (qemuDomainObjExitMonitor(driver, vm) < 0) {
    	virReportError(VIR_ERR_INTERNAL_ERROR, "%s", _("exit monitor failed"));
        goto free_devstr;
    }

    VIR_FREE(devstr);

    if (VIR_APPEND_ELEMENT(vm->def->vhostusers, vm->def->nvhostusers, vhostuser) < 0)
        return -1;
    VIR_DEBUG("AttachVhostUserLive, nvhostusers:%lu", vm->def->nvhostusers);
 ...
 exit_monitor:
    ...
    virReportError(VIR_ERR_INTERNAL_ERROR, "%s", _("attach vhost-user-blk chrdev failed exit monitor"));
 free_devstr:
    VIR_FREE(devstr);
 free_pci_addr:
    qemuDomainReleaseDeviceAddress(vm, &vhostuser->info, NULL);
    return ret;
}
                    ...
                    virDomainDeviceDefPtr dev)
{
    ...
    if (olddev->model && STREQ(olddev->model, "virtio") &&
        (olddev->driver.virtio.name != newdev->driver.virtio.name ||
         ...
         olddev->driver.virtio.host.standby != newdev->driver.virtio.host.standby ||
...
int
qemuDomainDetachVhostUserDevice(virDomainObjPtr vm,
                            virDomainVhostUserDefPtr def)
{
    ...
    ignore_value(virAsprintf(&def->info.alias, "%s", def->id));

    qemuDomainMarkDeviceForRemoval(vm, &def->info);

    qemuDomainObjEnterMonitor(driver, vm);
    if (qemuMonitorDelDevice(priv->mon, def->id)) { 
        virReportError(VIR_ERR_INTERNAL_ERROR, "%s", _("detach vhost-user-blk device failed"));
	ret = -1;
    }
    if (qemuDomainObjExitMonitor(driver, vm) < 0) {
        virReportError(VIR_ERR_INTERNAL_ERROR, "%s", _("qemuDomainObjExitMonitor failed"));
        ret = -1;
    } 
    ...
    VIR_DEBUG("detach vhost-user-blk device , qemuDomainWaitForDeviceRemoval ret:%d", ret);

    qemuDomainObjEnterMonitor(driver, vm);
    if (qemuMonitorDetachCharDev(priv->mon, def->id) < 0) {
        virReportError(VIR_ERR_INTERNAL_ERROR, "%s", _("detach vhost-user-blk chrdev failed"));
        ret = -1;
    }
    if (qemuDomainObjExitMonitor(driver, vm) < 0) {
    ...
    virReportError(VIR_ERR_INTERNAL_ERROR, "%s", _("check remove vhost-user-blk"));
    if (ret == -1)
        return ret;

    virReportError(VIR_ERR_INTERNAL_ERROR, "%s", _("remove vhost-user-blk"));
    if ((i = virDomainVhostUserDefFind(vm->def, def)) < 0) {
        virReportError(VIR_ERR_OPERATION_FAILED, "matching vhostuer device id:%s and path:%s not found",
                       def->id, def->path);
        return -1;
    }

    qemuDomainReleaseDeviceAddress(vm, &vm->def->vhostusers[i]->info, NULL);
    VIR_DELETE_ELEMENT(vm->def->vhostusers, i, vm->def->nvhostusers);
