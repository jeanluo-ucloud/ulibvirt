...
VIR_ENUM_IMPL(virDomainQemuCpu, VIR_DOMAIN_QEMU_CPU_LAST,
              "legacy-cache",
              "xlevel")
...
void virDomainDeviceDefFree(virDomainDeviceDefPtr def)
{
    ...
    case VIR_DOMAIN_DEVICE_IOMMU:
        ...
        break;
    case VIR_DOMAIN_DEVICE_VHOSTUSER:
        if (def->data.vhostuser && (def->data.vhostuser)->source)
            virDomainChrSourceDefFree((def->data.vhostuser)->source);
        if (def->data.vhostuser)
            VIR_FREE(def->data.vhostuser);
        break;
    case VIR_DOMAIN_DEVICE_VHOSTUSERMEM:
        VIR_FREE(def->data.vhostusermem);
        break;
...
virDomainDeviceGetInfo(virDomainDeviceDefPtr device)
{
    ...
    case VIR_DOMAIN_DEVICE_VHOSTUSER:
        return &device->data.vhostuser->info;
                                   ...
                                   void *opaque)
{
    ...
    device.type = VIR_DOMAIN_DEVICE_VHOSTUSER;
    for (i = 0; i < def->nvhostusers; i++) {
        device.data.vhostuser = def->vhostusers[i];
        if ((rc = cb(def, &device, &def->vhostusers[i]->info, opaque)) != 0)
            return rc;
    }
                        ...
                        unsigned int flags)
{
    ...
    if (def->type != VIR_DOMAIN_NET_TYPE_HOSTDEV &&
        ...
        if ((tmpNode = virXPathNode("./driver/host", ctxt))) {
            ...
            VIR_FREE(str);
            if ((str = virXMLPropString(tmpNode, "standby"))) {
                if ((val = virTristateSwitchTypeFromString(str)) <= 0) {
                    virReportError(VIR_ERR_CONFIG_UNSUPPORTED,
                                   _("unknown host standby mode '%s'"),
                                   str);
                    goto error;
                }
                def->driver.virtio.host.standby = val;
            }
            VIR_FREE(str);
        }
    ...
    if (virXPathUInt("string(./txqueuelen/@size)", ctxt, &def->txqueuelen) < -1) {
        virReportError(VIR_ERR_XML_ERROR, "%s",
                       _("malformed txqueuelen size"));
        goto error;
    }
...
static virDomainVhostUserDefPtr
virDomainVhostUserDefParseXML(xmlNodePtr node,
                          xmlXPathContextPtr ctxt)
{
    //struct stat filestat;
    virDomainVhostUserDefPtr vhost_user = NULL, ret = NULL;
    xmlNodePtr save = ctxt->node;
    char *tmp = NULL;

    ctxt->node = node;

    if (VIR_ALLOC(vhost_user) < 0)
        return NULL;

    vhost_user->type = VIR_DOMAIN_DEVICE_ADDRESS_TYPE_PCI;

    if (!(vhost_user->source = virDomainChrSourceDefNew(NULL)))
        goto error;

    if (!(tmp = virXMLPropString(node, "id"))) {
        virReportError(VIR_ERR_XML_ERROR, "%s",
                       _("missing vhost user id"));
        goto error;
    }
    strncpy(vhost_user->id, tmp, sizeof(vhost_user->id));
    VIR_FREE(tmp);

    if (!(tmp = virXMLPropString(node, "path"))) {
        virReportError(VIR_ERR_XML_ERROR, "%s",
                       _("missing vhost user path"));
        goto error;
    }
#if 0
    if (stat(tmp, &filestat) != 0)
    {
        virReportError(VIR_ERR_XML_ERROR, "stat vhost path:%s failed, please check %s exist",
                       tmp, tmp);
	goto error;
    }
#endif
    strncpy(vhost_user->path, tmp, sizeof(vhost_user->path));
    vhost_user->source->data.nix.path = tmp;

    if (!(tmp = virXMLPropString(node, "reconnect"))) {
        virReportError(VIR_ERR_XML_ERROR, "%s",
                       _("missing vhost user reconnect"));
        goto error;
    }
    vhost_user->reconnect = atoi(tmp);
    VIR_FREE(tmp);

    if (!(tmp = virXMLPropString(node, "queues"))) {
        virReportError(VIR_ERR_XML_ERROR, "%s",
                       _("missing vhost user queues"));
        goto error;
    }
    vhost_user->queues = atoi(tmp);
    VIR_FREE(tmp);
    vhost_user->source->type = VIR_DOMAIN_CHR_TYPE_UNIX;
    vhost_user->source->data.nix.listen = VIR_DOMAIN_CHR_SOURCE_MODE_CONNECT;
    vhost_user->source->data.nix.reconnect.enabled = VIR_TRISTATE_BOOL_YES;
    vhost_user->source->data.nix.reconnect.timeout = vhost_user->reconnect;

    VIR_DEBUG("id:%s, path:%s, reconnect:%d, queues:%d",
                       vhost_user->id, vhost_user->path, vhost_user->reconnect, vhost_user->queues);
    ctxt->node = save;
    ret = vhost_user;
    vhost_user = NULL;
...
static virDomainVhostUserMemDefPtr
virDomainVhostUserMemDefParseXML(xmlNodePtr node)
{
    struct stat filestat;
    virDomainVhostUserMemDefPtr vhost_user_mem = NULL, ret = NULL;
    char *tmp = NULL;

    if (VIR_ALLOC(vhost_user_mem) < 0)
        return NULL;

    if (!(tmp = virXMLPropString(node, "id"))) {
        virReportError(VIR_ERR_XML_ERROR, "%s",
                       _("missing vhost user memory id"));
        goto error;
    }
    strncpy(vhost_user_mem->id, tmp, sizeof(vhost_user_mem->id));
    VIR_FREE(tmp);

    if (!(tmp = virXMLPropString(node, "mempath"))) {
        virReportError(VIR_ERR_XML_ERROR, "%s",
                       _("missing vhost user memory path"));
        goto error;
    }
    if (stat(tmp, &filestat) != 0)
    {
        virReportError(VIR_ERR_XML_ERROR, "stat mempath:%s failed, please check %s exist",
                       tmp, tmp);
	goto error;
    }
    strncpy(vhost_user_mem->path, tmp, sizeof(vhost_user_mem->path));
    VIR_FREE(tmp);

    if (!(tmp = virXMLPropString(node, "size"))) {
        virReportError(VIR_ERR_XML_ERROR, "%s",
                       _("missing vhost user memory size"));
        goto error;
    }
    strncpy(vhost_user_mem->size, tmp, sizeof(vhost_user_mem->size));
    VIR_FREE(tmp);

    if (!(tmp = virXMLPropString(node, "align"))) {
        virReportError(VIR_ERR_XML_ERROR, "%s",
                       _("missing vhost user memory align"));
        goto error;
    }
    strncpy(vhost_user_mem->align, tmp, sizeof(vhost_user_mem->align));
    VIR_FREE(tmp);
                        ...
                        unsigned int flags)
{
    ...
    case VIR_DOMAIN_DEVICE_IOMMU:
        ...
        break;
    case VIR_DOMAIN_DEVICE_VHOSTUSER:
        if (!(dev->data.vhostuser = virDomainVhostUserDefParseXML(node, ctxt)))
            goto error;
        break;
    case VIR_DOMAIN_DEVICE_VHOSTUSERMEM:
        if (!(dev->data.vhostusermem = virDomainVhostUserMemDefParseXML(node)))
            goto error;
        break;
    case VIR_DOMAIN_DEVICE_NONE:
    case VIR_DOMAIN_DEVICE_LAST:
...
ssize_t
virDomainVhostUserDefFind(const virDomainDef *def,
                      const virDomainVhostUserDef *vhostuser)
{
    size_t i;

    for (i = 0; i < def->nvhostusers; i++) {
        if (strcmp(def->vhostusers[i]->id, vhostuser->id) == 0 && 
            strcmp(def->vhostusers[i]->path, vhostuser->path) == 0)
            return i;
    }
                     ...
                     unsigned int flags)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        case VIR_DOMAIN_FEATURE_APIC:
            ...
            ATTRIBUTE_FALLTHROUGH;
        case VIR_DOMAIN_FEATURE_QEMU_CPU:
    ...
    if (def->features[VIR_DOMAIN_FEATURE_QEMU_CPU] == VIR_TRISTATE_SWITCH_ON) {
        ...
        int value;
        node = ctxt->node;
        if ((n = virXPathNodeSet("./features/qemu_cpu/*", ctxt, &nodes)) < 0)
            goto error;

        for (i = 0; i < n; i++) {
            feature = virDomainQemuCpuTypeFromString((const char *)nodes[i]->name);
            if (feature < 0) {
                virReportError(VIR_ERR_CONFIG_UNSUPPORTED,
                               _("unsupported qemu cpu feature: %s"),
                               nodes[i]->name);
                goto error;
            }
            ...
            if ((value = virTristateSwitchTypeFromString(tmp)) < 0) {
                virReportError(VIR_ERR_CONFIG_UNSUPPORTED,
                               _("invalid value of state argument "
                                 "for qemu cpu feature '%s'"),
                               nodes[i]->name);
                goto error;
            }

            VIR_FREE(tmp);
            def->qemu_cpu_features[feature] = value;

            switch ((virDomainQemuCpu) feature) {
            ...
            case VIR_DOMAIN_QEMU_CPU_XLEVEL:
                ...
                if (tmp &&
                    virStrToLong_uip(tmp, NULL, 0, &def->qemu_cpu_xlevel) < 0) {
                    virReportError(VIR_ERR_XML_ERROR, "%s",
                                   _("invalid qemu cpu xlevel value"));
                    goto error;
                }
                VIR_FREE(tmp);
                break;
    ...
    /* analysis of the vhostuser devices */
    if ((n = virXPathNodeSet("./devices/vhostuser", ctxt, &nodes)) < 0)
        goto error;
    if (n && VIR_ALLOC_N(def->vhostusers, n) < 0)
        goto error;
    ...
    for (i = 0; i < n; i++) {
        ...
        ctxt->node = nodes[i];
        vhostuser = virDomainVhostUserDefParseXML(nodes[i], ctxt);
        if (!vhostuser)
            goto error;

        def->vhostusers[def->nvhostusers++] = vhostuser;
    }
    ctxt->node = node;
    VIR_FREE(nodes);

    /* analysis of the vhostusermem devices*/
    def->vhostusermem = NULL;
    if ((n = virXPathNodeSet("./devices/vhostusermem", ctxt, &nodes)) < 0)
        goto error;

    if (n > 1) {
        virReportError(VIR_ERR_INTERNAL_ERROR, "%s",
                       _("only a single vhostusermem device is supported"));
        goto error;
    } else if (n == 1) {
        virDomainVhostUserMemDefPtr vhostusermem =
            virDomainVhostUserMemDefParseXML(nodes[0]);
        if (!vhostusermem)
            goto error;
        def->vhostusermem = vhostusermem;
        VIR_FREE(nodes);
    }
                                      ...
                                      virDomainDefPtr dst)
{
    ...
    /* qemu cpu */
    if (src->features[VIR_DOMAIN_FEATURE_QEMU_CPU] == VIR_TRISTATE_SWITCH_ON) {
        for (i = 0; i < VIR_DOMAIN_QEMU_CPU_LAST; i++) {
            if (src->qemu_cpu_features[i] != dst->qemu_cpu_features[i]) {
                virReportError(VIR_ERR_CONFIG_UNSUPPORTED,
                               _("State of qemu cpu"
                                 "feature '%s' differs: "
                                 "source: '%s', destination: '%s'"),
                               virDomainQemuCpuTypeToString(i),
                               virTristateSwitchTypeToString(src->qemu_cpu_features[i]),
                               virTristateSwitchTypeToString(dst->qemu_cpu_features[i]));
                return false;
            }
            ...
            case VIR_DOMAIN_QEMU_CPU_XLEVEL:
                if (src->qemu_cpu_xlevel != dst->qemu_cpu_xlevel) {
                    virReportError(VIR_ERR_CONFIG_UNSUPPORTED,
                                   _("Qemu cpu xlevel differs: "
                                 ...
                                 virDomainNetDefPtr def)
{
    ...
    if (def->driver.virtio.host.standby) {
        virBufferAsprintf(&buf, "standby='%s' ",
                          virTristateSwitchTypeToString(def->driver.virtio.host.standby));
    }
    virBufferTrim(&buf, " ", -1);
                      ...
                      unsigned int flags)
{
    ...
    if (def->txqueuelen)
        virBufferAsprintf(buf, "<txqueuelen size='%u'/>\n", def->txqueuelen);
...
static int
virDomainVhostUserDefFormat(virBufferPtr buf,
                        virDomainVhostUserDefPtr def,
                        unsigned int flags)
{

    flags = flags;
    virBufferAsprintf(buf, "<vhostuser id='%s' path='%s' reconnect='%d' queues='%d'>\n",
                      def->id, def->path, def->reconnect, def->queues);
    virBufferAdjustIndent(buf, 2);
    virDomainDeviceInfoFormat(buf, &def->info, flags);
    virBufferAdjustIndent(buf, -2);
    virBufferAddLit(buf, "</vhostuser>\n");

    return 0;
}

static int
virDomainVhostUserMemDefFormat(virBufferPtr buf,
                        virDomainVhostUserMemDefPtr def,
                        unsigned int flags)
{

    flags = flags;
    virBufferAsprintf(buf, "<vhostusermem id='%s' mempath='%s' size='%s' align='%s'/>\n",
                      def->id, def->path, def->size, def->align);
                           ...
                           virDomainXMLOptionPtr xmlopt)
{
    ...
    if (i != VIR_DOMAIN_FEATURE_LAST ||
        ...
        for (i = 0; i < VIR_DOMAIN_FEATURE_LAST; i++) {
            ...
            case VIR_DOMAIN_FEATURE_QEMU_CPU:
                ...
                virBufferAddLit(buf, "<qemu_cpu>\n");
                virBufferAdjustIndent(buf, 2);
                for (j = 0; j < VIR_DOMAIN_QEMU_CPU_LAST; j++) {
                    if (def->qemu_cpu_features[j] == VIR_TRISTATE_SWITCH_ABSENT)
                        continue;

                    virBufferAsprintf(buf, "<%s state='%s'",
                                      virDomainQemuCpuTypeToString(j),
                                      virTristateSwitchTypeToString(
                                          def->qemu_cpu_features[j]));

                    switch ((virDomainQemuCpu) j) {
                    case VIR_DOMAIN_QEMU_CPU_LEGACY_CACHE:
                        break;

                    case VIR_DOMAIN_QEMU_CPU_XLEVEL:
                        if (def->qemu_cpu_features[j] == VIR_TRISTATE_SWITCH_ON)
                            virBufferAsprintf(buf, " value='%#x'",
                                              def->qemu_cpu_xlevel);
                        break;
    ...
    for (n = 0; n < def->nvhostusers; n++) {
        if (virDomainVhostUserDefFormat(buf, def->vhostusers[n], flags) < 0)
            goto error;
    }

    if (def->vhostusermem &&
	virDomainVhostUserMemDefFormat(buf, def->vhostusermem, flags) < 0)
        goto error;
                    ...
                    virCapsPtr caps)
{
    ...
    if (!virPidFileReadPath(priv->pidfile, &pid) && pid != obj->pid)
        return 0;
                       ...
                       virDomainXMLOptionPtr xmlopt)
{
    ...
    case VIR_DOMAIN_DEVICE_SHMEM:
        ...
        break;
    case VIR_DOMAIN_DEVICE_VHOSTUSER:
        rc = virDomainVhostUserDefFormat(&buf, src->data.vhostuser, flags);
        break;
