...
void vshOpenDefaultLogFile(vshControl *ctl);
void vshCloseDefaultLogFile(vshControl *ctl);

#define VIR_OPTS_STRING_BUFLEN 500
#define LOG_TYPE_RUN "Run"
#define LOG_TYPE_FIN "Fin"
#define LOG_TYPE_ERR "Err"
#define LOG_TYPE_QUIT "Quit"
/*
 *
 */
static int vshCmdOptDetail(const vshCmd *cmd, char *buf)
{
...
static void vshWriteCmdLog(vshControl *ctl, const char *type, const vshCmd *cmd, virDomainPtr dom) {
    char uuid[VIR_UUID_STRING_BUFLEN];
    const char *cmd_str = cmd->def->name;
    char opts[VIR_OPTS_STRING_BUFLEN] = {0};
    int len = 0;

    len = vshCmdOptDetail(cmd, opts);

    if (dom == NULL) {
        vshCmdLog(ctl, "%s cmd: virsh %s%s\n", type, cmd_str, opts);
    } else {
        if (virDomainGetUUIDString(dom, uuid) < 0) {
            vshCmdLog(ctl, "%s cmd: virsh %s%s with unknow UUID\n", type, cmd_str, opts);
        } else {
            vshCmdLog(ctl, "%s cmd: virsh %s %s%s\n", type, cmd_str, uuid, opts);
        }
    }
}

static bool vshCmdOptDomain(const vshCmd *cmd)
{
    const char *optname = "domain";
    bool found = false;
    const vshCmdOpt *opt;
...
virDomainPtr vshCommandGetDom(vshControl *ctl, const vshCmd *cmd)
{
    bool cmd_domain = vshCmdOptDomain(cmd);
    virDomainPtr dom = NULL;
    if (cmd_domain == true) {
        dom = vshCommandOptDomainBy(ctl, cmd, NULL, VIRSH_BYID | VIRSH_BYNAME | VIRSH_BYUUID);
    }
...
vshCommandRun(vshControl *ctl, const vshCmd *cmd)
{
    ...
    /*
     * Open default log file if none
     */
    if (ctl->logfile == NULL) {
        vshOpenDefaultLogFile(ctl);
    }
    ...
    while (cmd) {
        ...
        if ((cmd->def->flags & VSH_CMD_FLAG_NOCONNECT) ||
            ...
            dom = vshCommandGetDom(ctl, cmd);
            vshWriteCmdLog(ctl, LOG_TYPE_RUN, cmd, dom);
        ...
        if (STREQ(cmd->def->name, "quit") ||
            ...
            vshWriteCmdLog(ctl, LOG_TYPE_QUIT, cmd, dom);
            virDomainFree(dom);
            return ret;
        }
        ...
        vshWriteCmdLog(ctl, LOG_TYPE_FIN, cmd, dom);
        virDomainFree(dom);
        cmd = cmd->next;
    }
    return ret;
...
void
vshCmdLog(vshControl *ctl, const char *format, ...) {
    va_list ap;

    va_start(ap, format);
    vshOutputLogFile(ctl, VSH_ERR_NOTICE, format, ap);
    va_end(ap);
}
...
/**
 * vshOpenDefaultLogFile
 *
 * Open a default log file
 */
void
vshOpenDefaultLogFile(vshControl *ctl)
{
    ctl->logfile = vshStrdup(NULL, "/var/log/libvirt/virsh.log");
    vshOpenLogFile(ctl);
}
                 ...
                 va_list ap)
{
    ...
    */
    ...
    virBufferAsprintf(&buf, "[%d.%02d.%02d %02d:%02d:%02d.%03ld (UTC%s%ld) %s %d] ",
                      (1900 + stTm.tm_year),
                      (1 + stTm.tm_mon),
                      stTm.tm_mday,
                      stTm.tm_hour,
